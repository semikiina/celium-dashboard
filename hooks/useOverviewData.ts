 ...
 *
 * Fetches:
 *  - GET /api/nodes            — full node list with latest reading per node (5s refresh)
 *  - GET /api/network/stats    — pre-computed NetworkStats (30s refresh)
 *  - GET /api/alerts?resolved=false — active unresolved alerts (30s refresh)
 *
 * Returns:
 *  - nodes: Node[]
...
import { READINGS_REFRESH_INTERVAL, REFRESH_INTERVAL } from "@/lib/constants"
