...
async function seedReadings(
  ...
  for (const definition of definitions) {
    ...
    if (!insertedNode) {
      throw new Error(
        `Node "${definition.key}" not found when seeding readings.`
      )
    }
    ...
    for (let index = 0; index < READING_POINTS_PER_NODE; index += 1) {
      ...
      const rssi = Math.round(
        clamp(randomInRange(-108, -82) + signalBias, -120, -80)
      )
      const snr = round(
        clamp(randomInRange(-2, 10) + signalBias / 4, -5, 10),
        2
      )
  ...
  const rows: Record<string, unknown>[] = alerts.map((alert) => {
    ...
    const resolvedAt =
      alert.resolved && alert.resolvedAfterHours !== null
        ? new Date(
            createdAt.getTime() + alert.resolvedAfterHours * 60 * 60 * 1000
          )
        : null
